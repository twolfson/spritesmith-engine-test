...
exports.interpretBufferVinylImages = function (filepaths) {
  before(function interpretBufferVinylImagesFn (done) {
    // For each of our images, load them into memory
    var that = this;
    async.map(filepaths, fs.readFile, function handleBuffers (err, buffArr) {
      ...
      // Create buffer-based vinyl object
...
exports.interpretStreamVinylImages = function (filepaths) {
  before(function interpretStreamVinylImagesFn (done) {
    // For each of our images
    var that = this;
    var vinylArr = filepaths.map(function createVinylObj (filepath) {
      // Create a stream to read its contents
      var stream = fs.createReadStyle(filepath);

      // Generate a stream-based vinyl object
      return new Vinyl({
        path: filepath,
        contents: stream
      });
...
exports.interpretStreamNullImages = function (filepaths) {
  before(function interpretStreamNullImagesFn (done) {
    // For each of our images
    var that = this;
    var vinylArr = filepaths.map(function createVinylObj (filepath) {
      // Generate a null-based vinyl object
      return new Vinyl({
        path: filepath,
        contents: null
      });
